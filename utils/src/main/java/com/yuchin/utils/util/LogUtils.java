...
public final class LogUtils {
    ...
    private static String sDefaultDir;// log 默認存儲目錄
    private static String sDir;       // log 存儲目錄
    private static String sFilePrefix = "util";// log 文件前綴
    private static boolean sLogSwitch = true;  // log 總開關，默認開
    private static boolean sLog2ConsoleSwitch = true;  // logcat 是否打印，默認打印
    private static String sGlobalTag = null;  // log 標簽
    private static boolean sTagIsSpace = true;  // log 標簽是否為空白
    private static boolean sLogHeadSwitch = true;  // log 頭部開關，默認開
    private static boolean sLog2FileSwitch = false; // log 寫入文件開關，默認關
    private static boolean sLogBorderSwitch = true;  // log 邊框開關，默認開
    private static int sConsoleFilter = V;     // log 控制台過濾器
    private static int sFileFilter = V;     // log 文件過濾器
    private static int sStackDeep = 1;     // log 棧深度
    ...
    private static TagHead processTagAndHead(String tag) {
        if (!sTagIsSpace && !sLogHeadSwitch) {
            ...
            String className;
            // 混淆可能會導致獲取為空 加-keepattributes SourceFile,LineNumberTable
            if (fileName == null) {
                ...
                int index = fileName.indexOf('.');// 混淆可能導致文件名被改變從而找不到"."
    ...
    private static void printDeviceInfo(final String filePath) {
        ...
        final String head = "************* Log Head ****************" +
                ...
                "\nAndroid Version    : " + Build.VERSION.RELEASE +// 系統版本
